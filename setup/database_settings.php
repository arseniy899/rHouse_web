<?php
	...
	if($task == 'send'){
		...
		$database_charset	= isset($_POST['database_charset']) ? prepare_input($_POST['database_charset']) : '';	
		...
		//	$error_msg = lang_key('alert_db_password_empty');
			...
			if(empty($error_msg))
			{
				$_SESSION['config']['DB']['host'] = $database_host;
				$_SESSION['config']['DB']['dbname'] = $database_name;
				$_SESSION['config']['DB']['login'] = $database_username;
				$_SESSION['config']['DB']['password'] = $database_password;
				$_SESSION['config']['DB']['charset'] = $database_charset;
				...
				if($install_type == 'un-install'){
					...
					header('location: hub_secret.php');
				}
				exit;				
			}
		}		
	}else{
		$database_host		= isset($_SESSION['config']['DB']['host']) ? $_SESSION['config']['DB']['host'] : 'localhost';
		$database_name 		= isset($_SESSION['config']['DB']['dbname']) ? $_SESSION['config']['DB']['dbname'] : 'sh';
		$database_username	= isset($_SESSION['config']['DB']['login']) ? $_SESSION['config']['DB']['login'] : 'root';
		$database_password	= isset($_SESSION['config']['DB']['password']) ? $_SESSION['config']['DB']['password'] : 'qxR9poSO';
		$database_charset	= isset($_SESSION['config']['DB']['database_charset']) ? $_SESSION['config']['DB']['database_charset'] : 'utf8';
...
<div id="main">
	...
	<div id="content">
		<div class="central-part">
			...
			<tr>
				<td nowrap>&nbsp;<?php echo lang_key('database_charset'); ?>:</td>
				<td>
					<input type="text" class="form_text" name="database_charset" id="database_charset" size="30" value="<?php echo $database_charset; ?>" <?php if(EI_MODE != 'debug') echo 'autocomplete="off"'; ?>  />
				</td>
			</tr>
