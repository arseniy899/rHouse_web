			...
			$('#table').on('click', '.table-remove', function() 
			{
				...
				if (r) {
					if ($(this).attr("id"))
					{
						...
						xhttp.open("GET", API_URL+"table.values.delete.php?m=<?=$table;?>&id="+ $(this).attr("id"), true);
						xhttp.send();
					}
				}
			});
					...
					<tbody>
						...
						<tr id="rowf<?echo $id;?>">
							<?
								...
								?>
							<td>
								<form class="form-table" id="f<?=$id;?>" method="post" action=API_URL+"table.values.save.php?m=<?=$table;?>">
